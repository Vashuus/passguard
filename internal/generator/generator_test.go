...
func TestGenerateClasses(t *testing.T) {
	...
	if res.Entropy < o.MinEntropy {
		t.Errorf("entropy %.1f < %.1f for %q", res.Entropy, o.MinEntropy, pw)
	}
}
...
func TestGenerateNoSimilar(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		if strings.ContainsAny(pw, "1lIO0") {
			t.Errorf("similar character present: %q", pw)
		}
	}
}
...
func TestGenerateLowercaseOnly(t *testing.T) {
	...
	for i := 0; i < len(pw); i++ {
		if pw[i] < 'a' || pw[i] > 'z' {
			t.Errorf("non-lowercase char: %q", pw)
		}
	}
}
...
func TestRandomness(t *testing.T) {
	...
	b, _ := Generate(o)
	if a == b {
		t.Error("two generations identical (broken RNG?)")
	}
}
